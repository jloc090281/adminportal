...
import { setMenuList } from 'store/session/actions';
...
const ReportListScreen = ({ reportList, companyReports, setMenuList }) => {
  ...
  const addItem = item => {
    ...
    if (!companyReports.find(elm => elm.Id === item.id)) {
      setMenuList([...companyReports, selected], true);
    }
  };
...
const mapDispatchToProps = dispatch => {
  return bindActionCreators({ setMenuList }, dispatch);
};
